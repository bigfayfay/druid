...
## 二、内存估算公式：SQL/s × 并发 × SQL 长度 → JVM 参数

> req.md 核心问题：**是否可以依据 SQL/s + 并发数 + SQL 长度来估算 JVM 内存参数？**
> **答案：可以，但需要分两层估算——Transient（Eden，由吞吐量驱动）和 Persistent（Old Gen，由模板种类驱动）。**
...
```
总堆需求 = Eden 活跃区 + Survivor + Old Gen（LRU 缓存 + 框架对象）+ G1Reserve 缓冲
```

| 层级 | 决定因素 | 估算公式 | 特征 |
|------|---------|---------|------|
| **Eden（Transient）** | SQL/s × 并发 × SQL 长度 | 见下方 | 对象快速死亡，几秒内 GC |
| **Old Gen（Persistent）** | 模板种类数（Distinct Templates） | 非吞吐量驱动 | 累积不释放，决定堆下限 |

### 2.2 分配膨胀系数 K 的推导（核心参数）

SQL 解析过程每处理一个字节输入，堆上产生大量临时对象：

- **字符数组**：输入 SQL 被词法分析/LALR 解析多次拷贝
- **AST 节点**：每个 token/表达式对应对象（`SQLSelect`、`SQLExpr`、`SQLTableSource` 等），含 parent/children 双向引用
- **中间字符串**：标识符规范化、字面量提取产生临时 `String`
- **HashMap Entry**：缓存查找过程中的临时键值对

定义 **分配膨胀系数 K = 堆分配字节数 / SQL 原始字节数**。

**从监控数据回算 K（G1 默认 MaxGCPauseMillis=200，Young GC 间隔约 2~3 分钟）：**

```
node62（正常节点，NonSupport=0.06%）：
  R=1946 SQL/s, L=377 B, Eden committed=1.1 GB
  Eden 每周期有效使用≈1.0 GB, GC 间隔≈150s
  分配速率=1000 MB/150s≈6.7 MB/s → per-SQL=3.6 KB
  K=3.6 KB/0.377 KB≈9.5×

node203 交叉验证（NonSupport=0.17%）：
  R=1811 SQL/s, L=405 B → per-SQL≈3.6 KB
  K=3.6 KB/0.405 KB≈8.9×

node135 补充验证（最短 avgLen，NonSupport=0）：
  R=1967 SQL/s, L=198 B → per-SQL≈2.9 KB
  K=2.9 KB/0.198 KB≈14.6×
```

| 节点 | R | L | per-SQL | **K** |
|------|----|----|---------|-------|
| node62 | 1,946 | 377 B | ~3.6 KB | **9.5×** |
| node203 | 1,811 | 405 B | ~3.6 KB | **8.9×** |
| node135 | 1,967 | 198 B | ~2.9 KB | **14.6×** |
| **保守取整** | — | — | — | **K = 15** |

> **结论**：K ≈ **10~15**。取 **K=15** 为工程估算安全系数（覆盖复杂 SQL、GC 抖动、内存碎片）。
>
> **关键纠正**：分配速率公式为 `AR = R × L × K`，**不乘以线程数**。线程数增加的是并发度 C（同时处理的 SQL 数），不是单条 SQL 的分配量——每个线程同一时刻只处理一条 SQL，总分配速率仅取决于总吞吐量 R。

### 2.3 Eden 与 Young Gen 估算公式

```
分配速率       AR  = R × L × K                          [bytes/s]
Eden 容量      E   = AR × T_gc                          [bytes]
Survivor       S   = E × 0.15                           (两区合计 ~15%)
Young Gen      Y   = E + S

其中 T_gc = 目标 Young GC 间隔：
  - G1 MaxGCPauseMillis=50  → T_gc ≈ 45~90s（取 60s）
  - G1 MaxGCPauseMillis=200 → T_gc ≈ 120~300s
  - ZGC → 无需独立估算 Young Gen
```

**示例计算**：R=5000 SQL/s, L=500 B, K=15, T_gc=60s
```
AR = 5000 × 500 × 15 = 37,500,000 B/s ≈ 35.8 MB/s
E  = 35.8 × 60 = 2,148 MB ≈ 2.1 GB
S  = 2,148 × 0.15 ≈ 322 MB
Y  = 2,148 + 322 ≈ 2.5 GB
```
...
| 场景 | R | L | AR (K=15) | E (T=60s) | 实测 Eden committed | 评估 |
|------|---|---|-----------|-----------|-------------------|------|
| node62 | 1,946 | 377 | 10.5 MB/s | 630 MB | 1.1 GB | 有余量 ✓ |
| node203 | 1,811 | 405 | 10.5 MB/s | 630 MB | 1.1 GB | 有余量 ✓ |
| 10亿 中等 | 5,000 | 500 | 35.8 MB/s | 2.1 GB | — | L 档 4GB 够用 ✓ |
| **10亿 峰值** | **11,600** | 500 | **83.0 MB/s** | **4.9 GB** | — | ⚠️ **XL 6GB 偏紧** |

### 2.4 Old Gen 估算：非吞吐量驱动（关键）

**Old Gen 才是堆大小的决定性因素。** 与 SQL/s 无直接关系，而是由 **LRU 模板缓存** 规模决定：

```
Old Gen ≈ LRU 缓存大小 + Druid 框架对象(~100MB) + 解析器内部缓存
LRU 缓存 ≈ cache_limit × avg_template_entry_size

其中：
  cache_limit         = 配置的 LRU 最大条目数
  avg_template_entry  = SQL模板AST + 访问统计 ≈ avgLen × 3~8（缓存膨胀系数）
```

**实测 Old Gen 与 avgLen / 数据量的关系：**

| 节点 | 总数据量 | avgLen (成功) | Old Gen used | 推算每亿 Old Gen |
|------|---------|--------------|-------------|-----------------|
| node62 | 2 亿 | 377 | 422 MB | ~211 MB/亿 |
| node203 | 5 亿 | 405 | 562 MB | ~112 MB/亿 |
| node135 | 5 亿 | 198 | 664 MB | 高缓存命中，累积多 |
| node116 | 5 亿 | 778 | 432 MB | avgLen 高但不累积（NonSupport 多） |

> **规律**：数据量越大 → 模板种类覆盖越全 → Old Gen 趋于稳定。每亿数据 Old Gen 增量约 100~200MB，5 亿以上逐渐收敛。
...
其中 N_billion = 累计处理的 SQL 记录数（亿）
```

| 累计处理量 | 实测 Old Gen | 公式 O_MB | 误差 |
|-----------|-------------|----------|------|
...
代入推荐值 K=15, T_gc=60, ReservePercent=15%：
H_MB ≈ (R × L × 0.00099 + O_MB) / 0.85
```
...
**场景 B：已知硬件 → 反算可支撑吞吐量**
```
R_max ≈ ((H × 0.85 - O_MB) / 1.15) × 1048576 / (L × K × T_gc)
```

示例：16GB OS, Xmx=2GB (M 档), avgLen=500, 5 亿数据：
```
R_max ≈ ((2048 × 0.85 - 600) / 1.15) × 1048576 / (500 × 15 × 60)
     ≈ 1141 × 1048576 / 450000 ≈ 2658 SQL/s
```

**场景 C：运行后精调** — 从 GC 日志反推真实 K：
```
K_real = (EdenUsed_before_GC × GC_count) / (total_SQL × L)
```

### 2.8 与分档配置的对应

| 档位 | 适合的 SQL/s 范围 | avgLen 范围 | 公式推导 Xmx | 分档建议值 |
|------|------------------|-------------|-------------|-----------|
| S | < 2,000 | < 200 | ~1GB | 1g |
| M | 1,000~6,000 | 200~800 | 1.5~3GB | **2g** |
| L | 3,000~10,000 | 400~1500 | 3~5GB | **4g** |
| XL | > 8,000 | 任何 | 4~6GB | **6g** |

> **注意**：公式给出的是初始值。**最佳实践**：按公式粗估 → 按分档部署 → 运行后 `jstat -gc` 观察 Old Gen 实际占用 → 第 2 次调整缩/扩堆。监控数据 > 公式估算。
...
|------|---------|------|-----------|---------|---------|--------|
| **S** | 8 GB | 1 | 1~2 亿 | 1g / 1g | G1 | 1m |
| **M** | 16 GB | 4 | 1~5 亿 | 2g / 2g | G1 | 2m |
| **L** | 32 GB | 4 | 3~10 亿 | 4g / 4g | G1 | 2m |
| **XL** | 64~128 GB | 10 | 5~10 亿 | 6g / 6g | G1 / ZGC | 4m / ZGC无关 |
...
### 4.2 档位 S：8GB OS / 1 线程 / 1~2 亿

8GB 总内存下，JVM 堆固定 1GB。C 宿主进程 + OS 剩余 ~6.7GB。
...
### 4.4 档位 L：32GB OS / 4 线程 / 3~10 亿

32GB OS 有充足内存余量，堆固定 4GB 以容纳 10 亿级数据量下的 Old Gen 增长。
...
### 4.5 档位 XL：64~128GB OS / 10 线程 / 5~10 亿

10 线程下分配压力增大，堆固定 6GB。提供 G1 和 ZGC 两套方案。
...
### 5.1 MaxGCPauseMillis=50（核心参数，原 batch 方案=1000）
...
### 5.2 InitiatingHeapOccupancyPercent=35（默认 45）
...
### 5.3 G1ReservePercent=15（默认 10）

增加 5% 堆保留空间作为 evacuation 失败的缓冲。对于低延迟目标，宁可用 5% 堆空间换零 Full GC 的确定性。

### 5.4 Eden 上下限：G1NewSizePercent / G1MaxNewSizePercent
...
### 5.6 ParallelGCThreads / ConcGCThreads
...
|------|------------------|---|---|---|---------|----------|
| 堆 committed | 1.7~2.0 GB | 1.0 GB | 2.0 GB | 4.0 GB | 6.0 GB | 6.0 GB |
...
- 内存估算公式：H ≈ (R×L×15×60×1.15 + O) / 0.85
- 以 5000 SQL/s、avgLen=500 计算：H ≈ 3.9GB（L 档）
- 峰值 11600 SQL/s：H ≈ 7.9GB（XL 档 6GB 偏紧，建议 8GB）
...
8GB 中 JVM 堆 1GB + Metaspace 90MB + 线程栈 ~10MB + JVM 自身 ~200MB ≈ 0.8~1.3GB。剩 6.7GB 给 C 进程 + OS Cache。对于 1-2 亿数据/1 线程场景够用。如 C 进程自身内存需求大，可降为 Xmx=768m（S 档下限）。
