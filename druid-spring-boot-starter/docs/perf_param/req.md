...
## 目标
需要为 druid-ak 定义一些启动参数，以调优； 给出jvm参数的调优建议；

## 业务背景
druid-ak是依据alibaba的druid框架为依据， 做SQL解析得到SQL模板(预编译语句)的工具jar包。
1. c程序 将druid-ak.jar在一个环境中启动， 然后多线程调用（1-10个线程）；
2. 业务场景为数据库的操作语句审计
3. 业务数据量在1-2亿，以及3-5亿； 最大的情况可能会有10亿；依据设备硬件配置

### 要求
1. gc的时间尽量短，因为审计操作日志是实时的。
2. 线程启动个数OS的内存设置，8G-1线程，16G-4线程，32G-4线程，64G-128G启10线程

预计                                                                                                                                                                                                                             
JVM Heap(Xms/Xmx): 1.0 GB/2.0 GB | Heap(used/committed): 743.5 MB/1.9 GB | NonHeap(used): 89.9 MB | Free/Total: 1.2 GB/1.9 GB                                                                                                    
JVM Heap Pools: G1 Eden Space(used=75.0 MB, committed=1.1 GB, max=N/A) G1 Survivor Space(used=12.0 MB, committed=12.0 MB, max=N/A) G1 Old Gen(used=656.5 MB, committed=810.0 MB, max=2.0 GB)   
