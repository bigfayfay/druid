...
public class InsertTemplateStandardizer {
    public static void main(String[] args) {
        ...
        String template = InsertTemplateStandardizer.extractInsertTemplate(dirtySql, JdbcConstants.ORACLE);
        System.out.println("=== \u539f\u59cb SQL (\u7247\u6bb5) ===");
    ...
    public static String extractInsertTemplate(String sql, DbType dbType) {
        try {
            ...
            stmt.accept(new InsertNormalizeVisitor());
            return SQLUtils.toSQLString(stmt, dbType, null, VisitorFeature.OutputParameterized);
        }
        catch (Exception e) {
            e.printStackTrace();
            return sql;
    ...
    static class InsertNormalizeVisitor
    extends OracleASTVisitorAdapter {
        InsertNormalizeVisitor() {
        }
        ...
        public boolean visit(SQLInsertStatement x) {
            ...
            if (select != null && (query = select.getQuery()) instanceof SQLUnionQuery) {
                SQLSelectQueryBlock firstSelect = this.findFirstSelect((SQLUnionQuery)query);
                select.setQuery(firstSelect);
            }
            return true;
        }
        ...
        private SQLSelectQueryBlock findFirstSelect(SQLUnionQuery union) {
            ...
            if (left instanceof SQLUnionQuery) {
                return this.findFirstSelect((SQLUnionQuery)left);
            }
        ...
        public boolean visit(SQLNullExpr x) {
            SQLUtils.replaceInParent(x, new SQLVariantRefExpr("?"));
            return true;
        }
